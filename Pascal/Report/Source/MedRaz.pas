...
procedure TMedRaz.FormCreate(Sender: TObject);
var

  c: integer;

begin
  Dead := MedRaz.ADOTable1.RecordCount;
  SetLength(b, dead);
  SetLength(CB, dead);
  SetLength(M, dead);
  SetLength(T, dead);
    ...
    begin
     ...
     begin
      ...
      begin																						//создание Memo в количестве зависимом от количества записей в базе данных
        ...
        T[i].Left := 640;
        T[i].Top := 100 * (i mod c);
        T[i].Text := 'Тематика методической разработки';
        T[i].WordWrap := false;
      end;
    end;
         begin
      ...
      begin																						//создание Memo в количестве зависимом от количества записей в базе данных
        D[i] := TMemo.Create(nil);
        D[i].Parent := MedRaz;
        D[i].Tag := i + 1;
        D[i].Name := 'Memo' + (inttostr(i + 1));
        D[i].Width := 200;
        D[i].Height := 90;
        D[i].Left := 860;
        D[i].Top := 100 * (i mod c);
        D[i].Text := 'Срок выполнения';
        D[i].WordWrap := false;
      end;
    end;
  end;
...
procedure TMedRaz.CBSelect(Sender: TObject);													//добавление элементов из Combobox в memo
begin
...
procedure TMedRaz.N1Click(Sender: TObject);														//формирование отчетности в word
var
  index: integer;
  App: Variant;
begin
  ...
  app.ActiveDocument.Tables.Item(1).Cell(1, 2).Range.Text := 'По какой дисциплине';
  app.ActiveDocument.Tables.Item(1).Cell(1, 3).Range.Text := 'Тематика методической разработки';
  app.ActiveDocument.Tables.Item(1).Cell(1, 4).Range.Text := 'Срок выполнения';
  app.ActiveDocument.Tables.Item(1).Cell(1, 5).Range.Text := 'Отметка о выполнении';
  ...
  begin
    ...
    app.activedocument.tables.item(1).Cell(Index, 4).Range.Text := D[index - 2].Text;
